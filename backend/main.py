...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Or use env-based switching here
...
async def chat(request: ChatRequest):
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            data = response.json()
            print("OpenRouter /chat response:", data)  # Debug

        if "choices" not in data or not data["choices"]:
            raise HTTPException(status_code=500, detail=f"OpenRouter API error: {data}")
...
async def generate_tweet(request: TweetRequest):
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.post("https://openrouter.ai/api/v1/chat/completions", headers=headers, json=payload)
            data = response.json()
            print("OpenRouter /tweet response:", data)  # Debug
